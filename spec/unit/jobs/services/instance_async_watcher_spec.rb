...
module VCAP::CloudController
  module Jobs
    module Services
      describe InstanceAsyncWatcher do
        let(:broker) { service_instance.service_plan.service.service_broker }
        ...
        let(:handler) { double(:handler, run: nil) }
        ...
        describe '#initialize' do
          ...
          context 'when the caller does not provide the maximum duration' do
            it 'should use the default configuration value' do
        ...
        describe '#perform' do
          ...
          describe 'last_operation' do
            context 'when the returned state is `succeeded`' do
              let(:state) { 'succeeded' }
              ...
              it 'runs the handler' do
                ...
                expect(handler).to have_received(:run)
              end

              it 'should not enqueue another instance_async_watcher job' do
                job.perform
            ...
            context 'when the returned state is `failed`' do
              let(:state) { 'failed' }
              ...
              it 'fetches and updates the service instance state' do
                ...
                expect(db_service_instance.last_operation.description).to eq('failure description')
              end
            ...
            context 'when the returned state is `in progress`' do
              let(:state) { 'in progress' }
              let(:description) { 'in progress description' }
              ...
              it 'fetches and updates the service instance state' do
                ...
                expect(db_service_instance.last_operation.description).to eq('in progress description')
              end
              ...
              it 'should enqueue another job' do
                ...
                expect(Delayed::Job.first).to be_a_fully_wrapped_job_of(InstanceAsyncWatcher)
          ...
          describe 'exceptions' do
            context 'when saving to the database fails' do
              let(:state) { 'in progress' }
              before do
                allow_any_instance_of(ManagedServiceInstance).to receive(:save_and_update_operation).and_raise(Sequel::Error.new)
              end
          ...
          describe 'timing' do
            context 'when the job has run for more than the max poll duration' do
              let(:state) { 'in progress' }
              ...
              it 'should mark the service instance operation as failed' do
                ...
                expect(service_instance.last_operation.description).to eq('Service Broker failed to perform the operation within the required time.')
              end
            end
        ...
        describe '#job_name_in_configuration' do
          it 'returns the name of the job' do
            expect(job.job_name_in_configuration).to eq(:instance_async_watcher)
          end
        end
