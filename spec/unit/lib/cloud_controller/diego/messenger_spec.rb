  ...
  module Diego
    describe Messenger do
      ...
      describe '#send_stage_request' do
        let(:staging_guid) { StagingGuid.from(app) }
        let(:message) { { staging: 'message' } }

        before do
          allow(protocol).to receive(:stage_resource_request).and_return(message)
        ...
        it 'sends the staging message to the stager' do
          ...
          expect(protocol).to have_received(:stage_resource_request).with(app, staging_config)
